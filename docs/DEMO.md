...
| 1:20 | one data-quality trap, live | `\i demo/01_review_trap.sql` | "This order has two reviews in the file, with different scores. `review_id` is not even unique across orders. The reconciled layer keeps one per order, the most recent — 547 orders were affected." |
